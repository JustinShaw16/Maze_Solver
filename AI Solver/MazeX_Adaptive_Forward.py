...
def adaptive_astar(agent, target, grid):
    OPEN = []
    CLOSED = set()
    g = {agent: 0}
    h_values = {agent: distance(agent, target)}
    heapq.heappush(OPEN, (g[agent] + h_values[agent], agent))
    tree = {agent: None}
    ...
    while OPEN:
        ...
        for action in A(current):
            ...
            if grid.get(next_state) and not grid[next_state]['blocked'] and next_state not in CLOSED:
                ...
                if next_state not in g or tentative_g < g[next_state]:
                    g[next_state] = tentative_g
                    h_values[next_state] = distance(next_state, target)
                    f = tentative_g + h_values[next_state]
                    heapq.heappush(OPEN, (f, next_state))
                    tree[next_state] = current
    return None

def repeated_astar(agent, target, grid, max_iterations):
    for _ in range(max_iterations):
        path = astar(agent, target, grid)
        if path:
            return path
    return None
...
forward_path = repeated_astar(agent, target, grid, 100)
end_time = time.time()
...
if forward_path:
    ...
    print("Repeated Forward A*: No path found.")

#Adaptive A*
start_time = time.time()
adaptive_path = adaptive_astar(agent, target, grid)
end_time = time.time()
adaptive_runtime = end_time - start_time

if adaptive_path:
    print("\nAdaptive A*:")
    print("Shortest Path from agent to target:")
    print_path(adaptive_path, grid)
else:
    print("Adaptive A*: No path found.")


print_board(grid, maze_size)

print("\nRepeated Forward A* Runtime:", forward_runtime)
print("Adaptive A* Runtime:", adaptive_runtime)
